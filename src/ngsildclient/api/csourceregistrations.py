...
class CSourceRegistrations:
    ...
    def register(
        self, csourcereg: CSourceRegistration, raise_on_conflict: bool = True
    ) -> str:
        ...
        if raise_on_conflict:
            ...
            if conflicts:
                raise ValueError(
                    f"A csource already exists with same target : {conflicts}"
                )
    ...
    def query(
    ...
    ) -> Sequence[CSourceRegistration]:
        if type is None and attrs is None and q is None and georel is None:
            raise ValueError(
                "At least one of type, attrs, q or georel must be provided"
            )
        if georel is not None and (geometry is None or coordinates is None):
            raise ValueError(
                "geometry and coordinates must be provided if georel is provided"
            )
        ...
        if timeproperty:
            if timeproperty not in [
                ...
                raise ValueError(
                    "timeproperty must be one of observedAt, createdAt, modifiedAt or deletedAt"
                )
            params["timeproperty"] = timeproperty
        if timerel:
            if timerel not in ["before", "after", "between"]:
                raise ValueError(
                    "timerel must be one of before, after or between"
                )
